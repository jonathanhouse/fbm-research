...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
      integer(i4b), parameter     :: M=25,NT=2**M                 ! number of simulation time steps 
     
      real(r8b), parameter        :: GAMMA = 1.0D0                ! FBM correlation exponent 
      integer(i4b), parameter     :: NSETS = 32                  ! number of ensembles - want "ntasks" to be same or lower integer factor 
      integer(i4b), parameter     :: NWALKS_PER_SET = 64          ! number of walkers in an ensemble
      integer(i4b), parameter     :: NCONF=NSETS*NWALKS_PER_SET   ! number of walkers

      real(r8b), parameter        :: DECAYTIME = 1000.D0           ! decay time in memory kernel of P_int
      real(r8b), parameter        :: DECAYFACTOR = exp(-1.D0/DECAYTIME)	  
!      real(r8b), parameter        :: DECAYFACTOR = 1.D0            ! no decay
      ...
      real(r8b)                   :: force_weight = -2.0D0        ! multiplied against density gradient (negative as repelled by density)
      ...
      real(r8b),parameter         :: L = 40000.D0               ! Length of interval
      integer(i4b), parameter     :: NBIN =  200000              ! Number of bins in interval; 2*NBINS/L=10 works well for mean-density interaction
      ...
      logical,parameter           :: WRITEDISTRIB = .FALSE.        ! write out final distribution    
...
! Start of main loop - every process gets a collection of sets labelled by "iset" to compute ! 
      ...
      disorder_loop: do iset=1,totsets
          print *, 'iset=', iset,' dis. conf.=', iconf
#endif 
      ...
      if (myid .eq. 0) then 
            ...
            time_loop: do it=1, NT
				  ...
				  ! First scale distribution
				  conf_history(-NBIN:NBIN)= DECAYFACTOR*conf_history(-NBIN:NBIN)
				  ! Now add new walkerpositions
...
!!! AFTER PROC HANDLES ITS SETS, WE CAN COLLECT DATA FROM ALL MACHINES !!! 
      ...
      if (myid==0) then
        ...
        write(2,*) 'NT= ', NT
        write(2,*) 'L=', L
		write(2,*) 'DECAYFACTOR= ', DECAYFACTOR
