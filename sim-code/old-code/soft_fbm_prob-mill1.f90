...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#define PARALLEL
#define VERSION 'Soft fBM with p_accept weighted trajectories'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
      ...
      logical, parameter          :: SET_SUM_AVG = .FALSE.             ! if true, we average over the sum of walks from a set, rather 
                                                                      ! rather than average over each individual walker in all set 

      real(r16b), parameter        :: MAX_LOG_ACCEPT = -3421674.29088_r16b
      real(r16b), parameter        :: MAPPING_OFFSET = 10.0e50_r16b

                                                                              !paccept=0.8 : -4574259.3796647_r16b
									                        !paccept=0.9999: -2050.0203033_r16b

      real(r16b),parameter         :: mapping_factor = log(HUGE(p_accept)) - MAX_LOG_ACCEPT - log(MAPPING_OFFSET) ! 3422384.07359
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real(r8b)              :: ln_paccept_conf
      real(r8b)              :: grad                             ! density gradient 
      real(r16b)             :: conf_weight, sum_weight
      real(r8b)              :: force_step
...
! Time loop !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            time_loop: do it=1, NT
                  ...
                  if(config_history(ibin_new) .gt. config_history(ibin_curr)) then 
                        ln_paccept_conf = ln_paccept_conf + log(p_accept) ! p_accept becomes a factor smaller 
                  end if 
           ...
           ! a_max = largest ln_paccept_conf encountered 
           ! real_max = largest possible value for an 8byte real 
           ! m = mean ln_paccept_conf value 
           ! ai = i-th ln_paccept_conf value 
           ! exp(ai - m)

           !write(*,'(A,F0.7)') 'ln(p_accept)= ', ln_paccept_conf

           if (ln_paccept_conf > cutoff_accept) then 
            
		 
                  num_passed = num_passed + 1_i4b
                  !print *, 'data from ', iconf
                  temp_weight = exp(ln_paccept_conf + mapping_factor)
                  conf_weight = temp_weight + conf_weight
                  confxx(:) = temp_weight*xx(1:) + confxx(:)

                  if (.not. SET_SUM_AVG) then ! regular conf average 
                        conf2xx(:) = temp_weight*xx(1:)*xx(1:) + conf2xx(:)
                        conf4xx(:) = temp_weight*xx(1:)*xx(1:)*xx(1:)*xx(1:) + conf4xx(:)                   
                  end if 
		...
		do test_inc=1,NT
			...
			if ( .not. ieee_is_finite( temp_weight ) ) then 
				write(*, '(I9, A,E12.3,A,E12.3)') iconf, ": infinite weight=", temp_weight,&
				 " from ", ln_paccept_conf
			end if 
			if ( .not. ieee_is_finite( conf2xx(test_inc) ) ) then
				write(*, '(I9, A,I9, A, E12.3)') iconf, ": infinite conf2xx(", test_inc ,")=", conf2xx(test_inc)
			end if 
			if ( .not. ieee_is_finite( temp_weight*xx(test_inc)*xx(test_inc) ) ) then
				write(*, '(I9, A,I9, A, E12.3, A, E12.3,A, F0.7 )') iconf, ": infinite w*conf2xx(",& 
				test_inc ,")=", &
				exp(ln_paccept_conf + mapping_factor), '*', conf2xx(test_inc), ' ;p_accept=', ln_paccept_conf
				exit
			end if 
		
			if ( .not. ieee_is_finite( conf_weight ) ) then 
				write(*, '(I9, A,E12.3)') iconf, ": infinite conf_weight", conf_weight
			end if 
                  if (test_inc < 5) then 
                        write(*,'(A,I0.1,A,E13.6,A,E13.6,A,I0.1)') 'iconf=',iconf,':confxx=',&
                         confxx(test_inc), '|xx=', xx(test_inc), '|t=',test_inc
                  end if 
		end do 
      ...
      if (SET_SUM_AVG) then ! ASSUME SETS ARE EQUAL IN SIZE AND HAVE SUM OF WALKERS IN SET AS RV
            do it=1,NT 
                  sum2xx(it) = conf_weight*sumxx(it)*sumxx(it)
                  sum4xx(it) = conf_weight*sumxx(it)*sumxx(it)*sumxx(it)*sumxx(it)
            end do 
      end if 
...
! Now collect and analyze data !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (myid==0) then
            tlast=tnow
            call system_clock(tnow)
            write(*,'(A,I0,A,I0,A,I0,A,F0.3,A)') 'id=0: finished simulation - starting data collection'
      endif

      call MPI_REDUCE(confxx,  sumxx, NT, MPI_REAL16, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      call MPI_REDUCE(conf2xx, sum2xx, NT, MPI_REAL16, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      call MPI_REDUCE(conf4xx, sum4xx, NT, MPI_REAL16, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      call MPI_REDUCE(xxdis, sumdis, 2*NBIN+1, MPI_DOUBLE_PRECISION, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      call MPI_REDUCE(conf_weight, sum_weight, 1, MPI_REAL16, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
      call MPI_REDUCE(num_passed, sum_num_passed, 1, MPI_INTEGER, MPI_SUM, 0, MPI_COMM_WORLD, ierr)
	  ...
	  write(2,*) 'walkers above cutoff_accept=', sum_num_passed
        write(2,'(A,E13.6)') 'sum_weight=', sum_weight
       ...
       ! write(2,*) '<sum(grad)*sum(xix)>', sum_global/totconf
        ...
        it=1
        do while(it.le.NT)  
          Write(2,'(1X,I8,6(2X,E13.6))')  it, sumxx(it)/sum_weight, sum2xx(it)/sum_weight, sum4xx(it)/sum_weight
