...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
      ...
      !real(r8b),parameter         :: GAUS_BIN_SIGMA = 1.0D0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real(r8b)              :: force_step
      integer(i4b)           :: iconf, it, ibin, w, iset, iwalker, i , j                      ! configuration, and time counters   
      integer(i4b)           :: totconf,totsets                         ! actual number of confs
      real(r8b)              :: old_xx, conf_xix

      real(r8b)                :: conf_history(-NBIN:NBIN)       ! denisty histogram used for gradient calculations 
      real(r8b)                :: temp_xx(1:NWALKS_PER_SET, 0:NT)
...
!         print *, 'dis. conf.', iconf
      ...
      if (myid .eq. 0) then 
            ...
            temp_xx(:,0) = 0.D0 ! 1. Initialize starting positions for all walkers 
            conf_history(:) = 0.D0
            ...
            time_loop: do it=1, NT
                  ...
                  walkers_in_set: do iwalker=1,NWALKS_PER_SET
                        ...
                        conf_xix = walkers_xix(iwalker,it) ! store current walkers FBM step here 


                        if (it .le. T_BIN_TO_UNBIN) then ! if we're below this transition time, use binned-gradient appraoch

                              select case(GRAD_FORM)

                              case('ASYM5050')

                                    if( abs(ibin).le.(NBIN-GRAD_DX) ) then
                                          grad = ( conf_history(ibin+GRAD_DX) - conf_history(ibin) ) / (GRAD_DX*(LBY2/NBIN))
                                                if (rkiss05() .lt. 0.5D0) then
                                                      grad = ( conf_history(ibin) - conf_history(ibin-GRAD_DX) )&
                                                                                           / (GRAD_DX*(LBY2/NBIN))
                                                end if 
                              ...
                              case default 
                                    print *, "Invalid GRAD_FORM selected"
                              
                              end select 

                        else ! if we're above this time, switch to unbinned-gradient formula

                              select case(BIN_FORM)

                              case('GAUS')
                                    do i = 1,NWALKS_PER_SET 

                                          ! evaluate gradient at position of walker's current position -> (it-1)
                                          ! feed in all the trajectories up to that same time-step
                        ...
                        !! Find and store iwalker's current position 
                        old_xx = temp_xx(iwalker, it - 1)

                        !! Calculate the total step that will be added to the walker's trajectory 
                        if (it .le. T_FBM_OFF) then
                             total_step = conf_xix + force_step
                        else ! no FBM, purely gradient steps 
                             total_step = force_step
                        endif

                        !! set the walker's new position 
		            temp_xx(iwalker,it) = temp_xx(iwalker,it - 1) + total_step
                        ...
                        if (WALL .eq. 'SOFT') then
                              ...
                              if ( abs(temp_xx(iwalker,it)).gt.LBY2 ) then ! stopping boundaries
                                    temp_xx(iwalker,it) = temp_xx(iwalker,it - 1) ! undo the step just taken 
                              endif 
                        end if
                        ...
                        confxx(it)=confxx(it) + temp_xx(iwalker,it)*temp_xx(iwalker,it)
                        conf2xx(it)=conf2xx(it) + temp_xx(iwalker,it)*temp_xx(iwalker,it)*temp_xx(iwalker,it)*temp_xx(iwalker,it)

                        ibin=nint( temp_xx(iwalker,it)*NBIN/LBY2 ) ! get iwalker's new bin 
                  ...
                  if (it .le. T_BIN_TO_UNBIN) then ! after all walkers take step by current dist, update shared dist

                        do iwalker=1,NWALKS_PER_SET
                              ibin = nint(temp_xx(iwalker,it)*NBIN/LBY2)
...
!!! AFTER PROC HANDLES ITS SETS, WE CAN COLLECT DATA FROM ALL MACHINES !!! 
     ...
     if (myid.ne.0) then ! if not the parent, send data to the parent 
      ...
      if (myid==0) then
        ...
        write(2,*) 'KILL_FBM_TIME=', T_FBM_OFF
        write(2,*) 'T_BIN_TO_UNBIN', T_BIN_TO_UNBIN
...
!!!
      ...
      FUNCTION sum_gaus_derivs(x,traj,it) result(deriv)
            ...
            real(r8b),intent(in)   :: x ! evaluation point of the derivative 
            real(r8b),intent(in)   :: traj(0:it) ! trajectory of the current conf
            ...
            do i = 0, it
                  ...
                  deriv = deriv + & 
                  -2.0D0*(x - traj(i))/GAUS_BIN_SIGMA * exp( -(x - traj(i))**2 / GAUS_BIN_SIGMA )
